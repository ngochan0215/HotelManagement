...
import * as svcHelpers from "../services/serviceHelpers.js";
...
export const createGoodTicket = async (req, res) => {
  ...
  try {
    ...
    const employee = await svcHelpers.findEmployeeByUserId(container.eventBus, employee_id);
    if (!employee) {
      await session.abortTransaction();
...
export const createServiceUsage = async (req, res) => {
  ...
  try {
    ...
    let customer;
    try {
      booking = await svcHelpers.findBookingById(container.eventBus, booking_id);
      customer = await svcHelpers.findCustomerById(container.eventBus, customer_id);
    } catch (e) {
      await session.abortTransaction();
...
export const getAllServiceUsage = async (req, res) => {
  try {
    ...
    const serviceUsagesWithRooms = await svcHelpers.enrichServiceUsages(container.eventBus, serviceUsages);
...
export const getServiceUsageById = async (req, res) => {
  try {
    ...
    const enriched = await svcHelpers.enrichSingleServiceUsage(container.eventBus, serviceUsage);
    ...
    return res.status(200).json({
      success: true,
      data: {
        service_usage: enriched.service_usage,
...
export const confirmServiceUsage = async (req, res) => {
  ...
  try {
    ...
    const employee = await svcHelpers.findEmployeeByUserId(container.eventBus, employeeUserId);
    if (!employee) {
      await session.abortTransaction();
      return res.status(404).json({
        success: false,
