...
export class ServiceService {
    ...
    async createServiceCategory({ name, description }, files) {
        ...
        if (existing) throw new Error("Tên danh mục dịch vụ đã tồn tại.");
    ...
    async updateServiceCategory(id, { name, description }, files) {
        ...
        if (!category) throw new Error("Không tìm thấy danh mục loại dịch vụ.");
    ...
    async createService({ category_id, name, description, unit, price }, files) {
        ...
        if (existing) throw new Error("Đã tồn tại dịch vụ có cùng tên cho danh mục dịch vụ này.");
    ...
    async createGoodTicket({ import_date, goods_list }, userId) {
        ...
        try {
            const services = await this.Service.find({ _id: { $in: serviceIds } }).session(session);
            ...
            const ticket = await this.GoodTicket.create(
                [{ employee_id: employee._id, import_date, status: isToday ? "waiting_confirm" : "pending" }],
                { session }
            );
            ...
            await this.GoodImport.insertMany(detailDocs, { session });
            await session.commitTransaction();
            return { ticket: ticket[0], total_items: detailDocs.length };
        } catch (err) {
    ...
    async autoCreateGoodTicket({ import_date, goods_list, default_quantity = 10, default_price_percent = 0.8 }, userId) {
        ...
        try {
            ...
            const existing = await this.GoodTicket.findOne({ import_date: importDate, status: "pending" }).session(session);
            if (existing)
            ...
            const isToday = importDate.getTime() === today.getTime();
            const ticket = await this.GoodTicket.create(
                [{ employee_id: employee._id, import_date: importDate, status: isToday ? "pending" : "waiting_confirm" }],
                { session }
            );
            ...
            return { ticket: ticket[0], total_items: detailDocs.length, import_date: importDate, items_count: items.length, ticket_id: ticket[0]._id };
        } catch (err) {
            await session.abortTransaction();
            throw err;
        } finally {
    ...
    async updateGoodTicket(id, { import_date, goods_list }) {
        ...
        session.startTransaction();
        try {
            const ticket = await this.GoodTicket.findById(id).session(session);
            if (!ticket) throw new Error("Không tìm thấy phiếu nhập sản phẩm.");
            if (ticket.status !== "pending") throw new Error("Chỉ được sửa phiếu nhập ở trạng thái pending");
            ...
            if (goods_list) {
                ...
                await this.GoodImport.deleteMany({ ticket_id: ticket._id }, { session });
    ...
    async deleteGoodTicket(id, force) {
        ...
        try {
            ...
            const relatedImports = await this.GoodImport.find({ ticket_id: id }).session(session);
    ...
    async confirmGoodTicket(id, confirmerId) {
        ...
        try {
            ...
            const imports = await this.GoodImport.find({ ticket_id: ticket._id }).session(session);
            if (imports.length === 0) throw new Error("Phiếu nhập chưa có sản phẩm nào.");

            for (const item of imports) {
                const service = await this.Service.findById(item.service_id).session(session);
                if (!service) throw new Error("Có sản phẩm không tồn tại trong hệ thống.");
    ...
    async createServiceUsage({ booking_id, customer_id, services }, userId) {
        ...
        try {
            ...
            const dbServices = await this.Service.find({ _id: { $in: serviceIds }, status: "active" }).session(session);
            ...
            await this.UsageDetail.insertMany(usageDetailsData, { session });
            serviceUsage[0].total_fee = totalUsageFee;
            await serviceUsage[0].save({ session });
            await this.recalcServiceUsageStatus(serviceUsage[0]._id, session);
            await session.commitTransaction();

            return { service_usage: serviceUsage[0], usage_details: usageDetailsData };
        } catch (err) {
            await session.abortTransaction();
            throw err;
        } finally {
    ...
    async getServiceUsageById(id) {
        ...
        if (!serviceUsage) throw new Error("Không tìm thấy phiếu sử dụng dịch vụ");
    ...
    async deleteServiceUsage(id, force) {
        ...
        session.startTransaction();
        try {
            const serviceUsage = await this.ServiceUsage.findById(id).session(session);
            if (!serviceUsage) throw new Error("Không tìm thấy phiếu sử dụng dịch vụ");
            if (serviceUsage.status !== "pending")
                throw new Error("Chỉ được xóa phiếu sử dụng dịch vụ khi trạng thái là pending");

            const relatedDetails = await this.UsageDetail.find({ ticket_id: id }).session(session);
            ...
            await this.UsageDetail.deleteMany({ ticket_id: serviceUsage._id }, { session });
            await this.ServiceUsage.deleteOne({ _id: serviceUsage._id }, { session });
            await session.commitTransaction();
        } catch (err) {
            await session.abortTransaction();
            throw err;
    ...
    async updateServiceUsage(id, { services }) {
        ...
        try {
            ...
            await this.UsageDetail.deleteMany({ ticket_id: id }, { session });
            await this.UsageDetail.insertMany(usageDetailsData, { session });
            serviceUsage.total_fee = totalUsageFee;
            await serviceUsage.save({ session });
            await this.recalcServiceUsageStatus(serviceUsage._id, session);
            await session.commitTransaction();
    ...
    async recalcServiceUsageStatus(ticketId, session = null) {
    ...
    async confirmServiceUsage(id, employeeUserId) {
        ...
        try {
            ...
            const usageDetails = await this.UsageDetail.find({ ticket_id: id }).session(session);
            if (usageDetails.length === 0) throw new Error("Phiếu chưa có chi tiết sử dụng dịch vụ");

            const invalidDetail = usageDetails.find(d => d.status !== "waiting_confirm");
            if (invalidDetail) throw new Error("Phiếu có chi tiết không ở trạng thái waiting_confirm");
    ...
    async cancelServiceUsage(id, employeeUserId) {
        ...
        try {
            ...
            if (completedDetail) throw new Error("Không thể hủy phiếu vì có dịch vụ đã hoàn thành");
    ...
    async confirmUsageDetail(id, employeeUserId) {
        ...
        session.startTransaction();
        try {
            const usageDetail = await this.UsageDetail.findById(id).session(session);
            if (!usageDetail) throw new Error("Không tìm thấy usage detail");

            const serviceUsage = await this.ServiceUsage.findById(usageDetail.ticket_id).session(session);
            ...
            const svc = await this.Service.findById(usageDetail.service_id).session(session);
            if (svc.category_id.toString() === FOOD_CATEGORY_ID) {
