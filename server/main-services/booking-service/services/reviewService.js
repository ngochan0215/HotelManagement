...
export class ReviewService {
    ...
    //         const hoursPassed = (Date.now() - review.created_at.getTime()) / (1000 * 60 * 60);
    //         if (hoursPassed > EDIT_LIMIT_HOURS)
    //             throw new Error(`Không thể chỉnh sửa đánh giá sau ${EDIT_LIMIT_HOURS} giờ.`);

    //         // autofill if category_reviews is empty
    //         if (data.category_reviews && Array.isArray(data.category_reviews) && data.category_reviews.length > 0) {
    //             const RATING_FIELDS = ["room_quality", "cleanliness", "service", "value"];
    //             const seenCategoryIds = new Set();

    //             for (let i = 0; i < data.category_reviews.length; i++) {
    //                 const entry = data.category_reviews[i];
    //                 const prefix = `Loại phòng thứ [${i}] `;
    ...
    //         if (category_reviews || general_rating) {
    //             const reviewedCategoryIds = category_reviews.map((r) => r.category_id.toString());
    //             await Promise.all(
    //                 reviewedCategoryIds.map((category_id) => this.updateCategoryRating(category_id))
    //             );
    //         }
