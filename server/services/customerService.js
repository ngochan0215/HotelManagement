...
export class CustomerService {
    constructor({ Customer, User, PointsLog }) {
    ...
    getAllCustomers = async (query = {}) => {
        try {
            ...
            const customers = await this.Customer.find(filter)
    ...
    updateCustomer = async (id, updateData) => {
        try {
            ...
            if (email !== undefined) {
                const user = await this.User.findById(customer.user_id);
                    if (!user) {
                        throw new Error("Không tìm thấy tài khoản tương ứng.");
                    }

                if (email !== user.email) {
                    const existEmail = await this.User.findOne({ email });
                    if (existEmail) {
