...
export class DiscountService {
    constructor({Discount, Customer, Booking}) {
    ...
    async getDiscountById (discountId) {
        try {
            const discount = await this.Discount.findById(discountId).select("-__v");
            if (!discount) {
    ...
    async mapLoyaltyToTier(loyalty, bookingCount) {
    ...
    async checkDiscountAvailability(discount, customerId, orderValue) {
        ...
        if (conditions.customer_tiers && conditions.customer_tiers.length > 0) {
            const customer = await this.Customer.findById(customerId).select("loyalty booking_count");
            if (!customer) {
                throw new Error("Không tìm thấy thông tin khách hàng");
            }
            
            const customerTier = await this.mapLoyaltyToTier(customer.loyalty || "bronze", customer.booking_count || 0);
