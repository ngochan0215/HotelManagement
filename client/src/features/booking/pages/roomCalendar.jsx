...
const STATIC_ROOMS = [
  { _id: "r101", room_number: "101", type: "Standard", floor: 1 },
  { _id: "r102", room_number: "102", type: "Standard", floor: 1 },
  { _id: "r201", room_number: "201", type: "Deluxe", floor: 2 },
  { _id: "r202", room_number: "202", type: "Deluxe", floor: 2 },
  { _id: "r301", room_number: "301", type: "VIP", floor: 3 },
  { _id: "r302", room_number: "302", type: "VIP", floor: 3 },
  { _id: "r401", room_number: "401", type: "Family", floor: 4 },
  { _id: "r402", room_number: "402", type: "Family", floor: 4 },
];

const STATIC_EVENTS = [
  // 1. Đang ở (Occupied): Check-in hôm qua (14:00), Check-out mai (12:00)
  {
    _id: "e1", roomId: "r101", title: "Nguyễn Văn A",
    start: new Date(y, m, d - 1, 14, 0).toISOString(),
    end: new Date(y, m, d + 1, 12, 0).toISOString(),
    status: "occupied", price: 1500000
  },
  // 2. Đã đặt (Booked): Check-in hôm nay (14:00), Check-out mai (12:00)
  {
    _id: "e2", roomId: "r201", title: "Trần Thị B",
    start: new Date(y, m, d, 14, 0).toISOString(),
    end: new Date(y, m, d + 1, 12, 0).toISOString(),
    status: "booked", price: 2200000
  },
  // 3. Đã đặt (Booked): Check-in mai (14:00)
  {
    _id: "e3", roomId: "r301", title: "Lê Văn C",
    start: new Date(y, m, d + 1, 14, 0).toISOString(),
    end: new Date(y, m, d + 3, 12, 0).toISOString(),
    status: "booked", price: 5000000
  },
  // 4. Dọn dẹp (Cleaning): Hôm nay 12:00 -> 14:00
  {
    _id: "e4", roomId: "r102", title: "Dọn dẹp",
    start: new Date(y, m, d, 12, 0).toISOString(),
    end: new Date(y, m, d, 14, 0).toISOString(),
    status: "cleaning", price: 0
  },
  // 5. Bảo trì (Maintenance): Cả ngày hôm nay
  {
    _id: "e5", roomId: "r402", title: "Sửa điều hòa",
    start: new Date(y, m, d, 8, 0).toISOString(),
    end: new Date(y, m, d, 17, 0).toISOString(),
    status: "maintenance", price: 0
  },
  // 6. Đang ở (Occupied): Check-in trưa nay 12:30 (Khách quen)
  {
    _id: "e6", roomId: "r202", title: "Phạm Văn D",
    start: new Date(y, m, d, 12, 30).toISOString(),
    end: new Date(y, m, d + 2, 12, 0).toISOString(),
    status: "occupied", price: 2800000
  }
];
...
export default function RoomCalendar() {
  ...
  const filteredRooms = useMemo(() => {
    return rooms.filter(room => {
      const matchName = room.room_number.toLowerCase().includes(searchTerm.toLowerCase());
      const matchType = filterType === 'all' || room.type === filterType;

      let matchStatus = true;
      if (filterStatus === 'available') {
        // Phòng trống = Không có event nào đè lên ngày hiện tại
        // (Logic đơn giản: check xem có event nào của phòng đó trong list không)
        matchStatus = !events.some(e => e.roomId === room._id);
      } else if (filterStatus !== 'all') {
        matchStatus = events.some(e => e.roomId === room._id && e.status === filterStatus);
      }
      return matchName && matchType && matchStatus;
    });
  }, [rooms, events, searchTerm, filterType, filterStatus]);

  const uniqueTypes = useMemo(() => [...new Set(rooms.map(r => r.type))], [rooms]);
                        ...
                        {filteredRooms.map((room) => (
                            <div key={room._id} className="flex border-b border-gray-200 hover:bg-gray-50/50 transition-colors group relative" style={{ height: ROW_HEIGHT }}>
                                <div className="sticky left-0 z-30 bg-white border-r border-gray-300 flex flex-col justify-center px-4 shrink-0 group-hover:bg-gray-50 transition-colors shadow-[4px_0_10px_-5px_rgba(0,0,0,0.05)]" style={{ width: ROOM_COL_WIDTH, minWidth: ROOM_COL_WIDTH }}>
                                    ...
                                    <div className="flex items-center gap-2"><span className={`text-[10px] px-2 py-0.5 rounded font-medium border bg-gray-50 text-gray-600 border-gray-100`}>{room.type}</span></div>
                                </div>
                    ...
                    <div className="p-6 space-y-4">
                        ...
                        <div className="pt-4 grid gap-3">
                            {selectedEvent.status === 'booked' && (
                                <>
                                    <button onClick={handleCheckIn} className="w-full flex items-center justify-center gap-2 bg-emerald-600 hover:bg-emerald-700 text-white py-2.5 rounded-xl font-bold transition shadow-lg shadow-emerald-200">
                                        <FiLogIn /> Nhận Phòng (Check-in)
                                    </button>
                                    <button onClick={handleCancel} className="w-full flex items-center justify-center gap-2 bg-red-50 hover:bg-red-100 text-red-600 py-2.5 rounded-xl font-bold transition">
                            ...
                            {selectedEvent.status === 'occupied' && (
                                <>
                                    <button onClick={() => alert("Mở form thêm dịch vụ...")} className="w-full flex items-center justify-center gap-2 bg-indigo-50 hover:bg-indigo-100 text-indigo-700 py-2.5 rounded-xl font-bold transition">
                                        <FiShoppingCart /> Thêm Dịch Vụ
                                    </button>
                                    <button onClick={handleCheckOut} className="w-full flex items-center justify-center gap-2 bg-orange-500 hover:bg-orange-600 text-white py-2.5 rounded-xl font-bold transition shadow-lg shadow-orange-200">
                                        <FiLogOut /> Trả Phòng (Check-out)
                                    </button>
                                </>
                            )}

                            {(selectedEvent.status === 'cleaning' || selectedEvent.status === 'maintenance') && (
